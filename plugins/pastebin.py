...
syntax = {".sh": "bash", ".c": "c", ".h": "c", ".cpp": "cpp", ".cs": "csharp", ".css": "css", ".html": "html",
...
parser.add_option("-s", "--syntax", dest="syntax", metavar="FORMAT",
                  help="Specify syntax format (common types will be guessed from filename otherwise, use -s none to "
                       "stop this)")
...
parser.add_option("-i", "--interactive", dest="interactive", default=False, action="store_true",
                  help="Interactive mode - echoes everything received on stdin to stdout so programs which require "
                       "console input can be used piped into Pastebin")
...
parser.add_option("-k", "--key", dest="getkey", action="store_true", default=False,
                  help="Return user key to avoid giving username and password in future")
...
def get_user_key():
    ...
    try:
        reply = urlrequest.urlopen("http://pastebin.com/api/api_login.php", urlparse(request))
...
if not args:
    ...
    for i in args:
        with open(i) as f:
            ...
            if (len(args) is 1) & (options.name is not None):
                add["api_paste_name"] = options.name
            if options.syntax:
                add["api_paste_format"] = options.syntax.lower()
            else:
...
for target in targets:
    ...
    # print target
    try:
        req = urlrequest.urlopen("http://pastebin.com/api/api_post.php", urllibparse.urlencode(target))
    except urlrequest.URLError:
        if not options.quiet:
            print("Error uploading", filename + ":", "Network error")
        exit(2)
